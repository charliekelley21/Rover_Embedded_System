...
void MQTTClientCallback(int32_t event, void *metaData, uint32_t metaDateLen, void *data, uint32_t dataLen)
{
    ...
    switch((MQTTClient_EventCB)event)
    {
        ...
        case MQTTClient_RECV_CB_EVENT:
        {
            ...
            // copy topic into our struct
            memcpy(queueElement.topic, receivedMetaData->topic, receivedMetaData->topLen);
            queueElement.topic[receivedMetaData->topLen] = 0;

            // don't try to overfill our buffer
            if (dataLen > PAYLOAD_SIZE)
                dbgErrorRoutine(ERROR_MSG_RECV_TOO_BIG);

            // feel like this is all we need. who cares about the topic?
    ...
    // send using our queue
    sendMsgToQueue(mqtt_qh, &queueElement);
}
...
void *MQTTAppThread(void *threadParams)
{
    ...
    while(1)
    {
        queueElement = receiveData(mqtt_qh);
