...
// toggle LED to show we're good (status basically)
void timer2LEDCallback(Timer_Handle myHandle)
{
    static int dbg_msg_count = 0;
    static struct msgQueue to_pub;
    to_pub.event = ANALYTICS_PUB;

    dbg_msg_count ++;
    GPIO_toggle(CONFIG_GPIO_LED_0);

    // publish at ~1Hz (70ms * 14)
    if (dbg_msg_count % 14 == 0)
    {
        dbg_msg_count = 0;
        dbg_msg.sequence ++;
        to_pub.data_len = encode_status(to_pub.payload, dbg_msg);
        sendMsgToQueue(qh, &to_pub);
    }
}
...
void mainThread(void * args)
{
    ...
    // let timer2 toggle an LED forever, quickly, so we know we're good
    Timer_Params_init(&params);
    params.period = 70000;
    params.periodUnits = Timer_PERIOD_US;
    params.timerMode = Timer_CONTINUOUS_CALLBACK;
    params.timerCallback = (Timer_CallBackFxn)timer2LEDCallback;
    ...
    MQTTClient_Handle mqttClientHandle = MQTT_IF_Connect(mqttClientParams, mqttConnParams, MQTT_EventCallback);
    ...
    ret |= MQTT_IF_Subscribe(mqttClientHandle, "lidar/start_scan", MQTT_QOS_0, taskLIDAR_CB);
    ...
    // read into this struct, and publish as necessary
